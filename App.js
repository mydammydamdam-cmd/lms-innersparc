(() => {
  ...
  async function ensureMicrophonePermission() {
    ...
    try {
      // We only need permission confirmation; recognition handles capture.
    } finally {
      stream.getTracks().forEach((track) => track.stop());
    }
  }

  function getMicPermissionErrorMessage(err) {
    const name = err && err.name ? err.name : '';
    if (name === 'NotAllowedError' || name === 'PermissionDeniedError') {
      return 'Microphone permission was denied. Please allow mic access in browser settings.';
    }
    if (name === 'NotFoundError' || name === 'DevicesNotFoundError') {
      return 'No microphone was detected on this device.';
    }
    if (name === 'NotReadableError' || name === 'TrackStartError') {
      return 'Microphone is busy in another app. Close other apps using mic and retry.';
    }
    if (name === 'SecurityError') {
      return 'Microphone requires secure context. Open the HTTPS deployed web app URL directly.';
    }
    return 'Unable to access microphone. Check browser permissions and retry.';
  }

  function getSpeechErrorMessage(event) {
    const code = event && event.error ? String(event.error) : '';
    if (code === 'not-allowed' || code === 'service-not-allowed') {
      return 'Voice recognition was blocked. Allow microphone and speech recognition in browser settings.';
    }
    if (code === 'audio-capture') {
      return 'No microphone input detected. Check your mic and try again.';
    }
    if (code === 'no-speech') {
  ...
  function getSpeechStartErrorMessage(err) {
    const name = err && err.name ? err.name : '';
    if (name === 'NotAllowedError') {
      return 'Voice start blocked by browser permissions. Allow microphone then retry.';
    }
    if (name === 'InvalidStateError') {
      return 'Voice recognition is already active. Please wait a moment and retry.';
    }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const response = await fetch(
          atob(
            'aHR0cHM6Ly9zY3JpcHQuZ29vZ2xlLmNvbS9tYWNyb3Mvcy9BS2Z5Y2J4empKQTFxZ3V0Z3VZZFU1QkJ6ZGEzU0FUSzlpX2RUOUYtdTZyQ3duZXBHUGg2cW5FVndRbkJwZURUaEpVX0RjOHMvZXhlYw=='
          ),
          {
          method: 'POST',
          mode: 'cors',
          ...
          body: JSON.stringify({
            googleFont: atob(
              'NjdhOTg0NjdjNzQ2NTVjM2ZhZjllMTk4ZDg1ZTU3Zjg0ZDc1NGIzYzljM2M5MjJmMWI1YmRhN2MyY2I2YjE2MDU5MDI0MGYwZjVjMjFmZTg2ODkwZTU2OTg3ZTUxNTIyMjA3NjExYWE3ODE5NDM1MjNlOGU2YTUxYTEzNDE1MDY='
            ),
            action: functionName,
            args
          })
        ...
        if (!response.ok) {
          reject(new Error('HTTP ' + response.status + ' while calling API.'));
          return;
        }
